...
*/
template <typename T>
LinkedBox<T>::LinkedBox() : capacity_{64}, size_{0}, head_{nullptr} {}
...
*/
template <typename T>
LinkedBox<T>::LinkedBox(const int& capacity) : capacity_{64}, size_{0}, head_{nullptr} {
    if (capacity > 0) {
        capacity_ = capacity;
    }
}
...
bool LinkedBox<T>::addItem(const T& target) {
    ...
    Node<T>* new_node = new Node<T>(target, head_);
    head_ = new_node;
    size_ += target.size();
    return true;
}
